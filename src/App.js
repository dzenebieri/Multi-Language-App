...
function App() {
  ...
  const [isMLAppULShow, setMLAppULShow] = useState(false);
  const toggleMLAppULShow = () => {
    setMLAppULShow(!isMLAppULShow);
  };

  return (
    <div className='MLAppDivCS'>
      <div className='MLAppDivCSN'>
      ...
      <div className='MLAppPDivCS'>
        <p style={{ fontSize: 18, margin: '18px 0' }}> {t('welcomeMessage')} </p>
      </div>
    </div>
  );
}
